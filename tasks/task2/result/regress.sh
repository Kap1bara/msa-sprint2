...
# Предварительные бронирования для тестов - через REST чтобы прокинулось в сервис
curl -sS -X POST "http://monolith:8080/api/bookings?userId=test-user-2&hotelId=test-hotel-1&promoCode=TESTCODE1"
curl -sS -X POST "http://monolith:8080/api/bookings?userId=test-user-3&hotelId=test-hotel-1&promoCode="
